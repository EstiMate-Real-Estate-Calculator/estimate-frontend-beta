...
export async function removeTutorialFromAllUsers(tutorialNumber) {
  // Get all users from your data source.
  const users = await userHandler.getAllUsers();

  for (const user of users) {
    // Retrieve the current tutorials array (or default to an empty array).
    let tutorials = user.tutorials || [];

    // Check if the user has the tutorial we want to remove.
    if (tutorials.includes(tutorialNumber)) {
      // Filter out the specified tutorialNumber.
      tutorials = tutorials.filter((num) => num !== tutorialNumber);

      // Update the user record with the revised tutorials array.
      await userHandler.updateUser(user.id, { tutorials });
      console.log(`Removed tutorial ${tutorialNumber} from user ${user.id}`);
    }
  }
}